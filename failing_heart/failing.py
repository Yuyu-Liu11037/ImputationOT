...
parser.add_argument("--source_batches", type=str, default="1", help="Impute batch range from 1 to 45")
parser.add_argument("--target_batch", type=int, default=20, help="Batch number to impute")

parser.add_argument("--wandb_group", type=str, default="failing")
...
device = 'cuda:0'
batch_sizes = [2189, 2007, 1972, 1649, 1634, 1614, 1593, 1476, 1458, 1443, 1440, 1422, 1411, 1381, 1380, 1329, 1277, 1208, 1115, 1092, 1080, 1060, 1057, 1032, 1013, 994, 977, 964, 958, 949, 941, 851, 848, 766, 742, 725, 724, 719, 700, 653, 551, 499, 429, 371, 307]
...
if args.use_wandb is True:
    wandb.init(
        ...
        config={
            "dataset": "failing_heart",
            "epochs": args.epochs,
            "h_loss weight": args.aux_weight
        }
    )

failing = ad.read_h5ad("/workspace/ImputationOT/data/failing.h5ad") 
failing.var_names_make_unique()
random_indices = np.random.choice(failing.shape[0], 50000, replace=False)
failing = failing[random_indices, :]

#####################################################################################################################################
print("Start preprocessing")
### preprocess
### step 1: normalize
sc.pp.normalize_total(failing, target_sum=1e4)
### step 2: log transform
sc.pp.log1p(failing)
### step 3: select highly variable features
sc.pp.highly_variable_genes(failing, subset=True)
    ...
    if epoch == 0 and args.use_wandb is True:
        ...
        for i in range(1, len(batch_sizes) + 1):
            if i in source_batches:
                ...
                tmp = X[batch_sizes_cumsum[i-1]:batch_sizes_cumsum[i]].detach().cpu().numpy()
            X_full.append(tmp)
        failing.X = np.vstack(X_full)
        ari, nmi, purity, jaccard = tools.cluster_with_leiden(failing, resolution_values=args.resolution_values)
