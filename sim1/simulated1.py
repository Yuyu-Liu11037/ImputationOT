...
device = 'cuda:0'
batch_sizes = [2908, 2422, 2120, 1929, 1761, 957]
...
simulated = ad.read_h5ad("/workspace/ImputationOT/data/sim1_norm.h5ad") 
simulated.var_names_make_unique()

#####################################################################################################################################
# print("Start preprocessing")
# ### preprocess
# ### step 1: normalize
# sc.pp.normalize_total(simulated, target_sum=1e4)
# ### step 2: log transform
# sc.pp.log1p(simulated)
# ### step 3: select highly variable features
# sc.pp.highly_variable_genes(simulated, subset=True)
# print(f"Finish preprocessing\n")
#####################################################################################################################################

X = simulated.X.toarray()
    ...
    if epoch == 0 and args.use_wandb is True:
        ...
        for i in range(1, len(batch_sizes) + 1):
            if i in source_batches:
                ...
                tmp = X[batch_sizes_cumsum[i-1]:batch_sizes_cumsum[i]].detach().cpu().numpy()
            X_full.append(tmp)
        simulated.X = np.vstack(X_full)
        ari, nmi, purity, jaccard = tools.cluster_with_leiden(simulated, resolution_values=args.resolution_values)
