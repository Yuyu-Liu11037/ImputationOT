...
X = torch.cat((X[:47025], X[-22224:]), dim=0)   # Matrix is too large. Remove certain rows to save memory.
ground_truth = X.clone()

mask = torch.zeros(X.shape, dtype=torch.bool).to(device)
mask[-22224:, :4000] = True   # mask X(4,1)

nonzero_mask1121 = (X[:47025, :4000] != 0).to(device)   # nonzero data of X(1,1), X(2,1)
...
mean_values = torch.sum(X[:47025, :4000], dim=0) / torch.sum(nonzero_mask1121, dim=0)
